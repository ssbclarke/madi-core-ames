...
import { SOURCE_ANALYSIS_PROMPT } from "../analysis.prompt.js";
import { SourceStore } from "../../../storage/source.store.js";
import { sourceSummaryStringifier } from "./sourceSummaryStringifier.js";
...
class SummarizeSourceChain extends DefaultChain{
    
    async _call(input){
        debug('SummarizeSourceChain')
        const record = setupRecorder({mode:process.env.NOCK_MODE});
        const { completeRecording } = await record('summarizeSourceChain');
        ...
        if(!source?.metadata?.summary){
            ...
            source = await sourceStore.patch(source.id, source)
        }
...
export const summarizeSourceChain = new SummarizeSourceChain({
    name: 'summarizeSourceChain',
    inputVariables: ["summarizedDocsJson"],
    outputKey: "summarizedSourceString",
})
